...
import type { Question, AnswerFeedback } from "@/lib/api-types";
...
const QuestionPanel = ({ questions, submissionId }: QuestionPanelProps) => {
  ...
  if (!questions || questions.length === 0) {
    return (
      <div className="qlc-card h-full flex items-center justify-center p-8">
        <div className="text-center max-w-sm">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-4">
            Submit your Python code to generate AI-powered comprehension
            questions.
          </p>
  ...
  const handleSubmitAnswer = async () => {
    ...
    setSubmitting(true);
    try {
      const answerValue: string | number =
      ...
      setFeedback({ ...feedback, [currentQuestion.id]: result.feedback });
    } catch (e) {
      toast({
        title: "Error",
        description: (e as Error).message || "Failed to submit answer",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  const getDifficultyBadgeClass = (difficulty: string) => {
    ...
    return map[difficulty] || "badge-slate";
  };
  ...
  return (
    <div className="qlc-card h-full flex flex-col overflow-hidden">
      {/* Header */}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {answeredCount > 0 && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800">
              <Trophy
                className={`w-4 h-4 ${correctCount === answeredCount ? "text-amber-500" : "text-slate-400"}`}
              />
          ...
          {questions.map((q, idx) => {
            ...
            return (
              <button
                ...
                title={`Question ${idx + 1}${answered ? (answered.is_correct ? " - Correct" : " - Incorrect") : ""}`}
              />
            );
          })}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-6">
          ...
          {currentQuestion.context &&
            typeof currentQuestion.context === "string" && (
          ...
          <div className="space-y-4">
            ...
            >
              <div className="flex items-start gap-4">
                <div
                  className={`p-2 rounded-xl ${currentFeedback.is_correct ? "bg-emerald-500" : "bg-rose-500"}`}
                >
                  {currentFeedback.is_correct ? (
                ...
                <div className="flex-1">
                  ...
                  {currentFeedback.correct_answer != null &&
                    !currentFeedback.is_correct && (
            ...
            {/* Detailed Explanation (from Answer Explainer LLM) */}
              ...
              <div className="border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
                ...
                {showDetailedExplanation[currentQuestion.id] && (
                  <div className="p-5 space-y-4 bg-white dark:bg-slate-900">
                    ...
                    {currentQuestion.answer_explanation.code_references
                      ?.length > 0 && (
                      <div>
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2 flex items-center gap-2">
                        ...
                        <ul className="space-y-1 pl-6">
                          {currentQuestion.answer_explanation.code_references.map(
                            (ref, idx) => (
                              <li
                                key={idx}
                                className="text-sm text-slate-600 dark:text-slate-400 font-mono bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-lg"
                              >
                                {ref}
                              </li>
                            )
                    ...
                    {currentQuestion.answer_explanation.analysis_references
                      ?.length > 0 && (
                      <div>
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2 flex items-center gap-2">
                        ...
                        <ul className="space-y-1 pl-6">
                          {currentQuestion.answer_explanation.analysis_references.map(
                            (ref, idx) => (
                              <li
                                key={idx}
                                className="text-sm text-slate-600 dark:text-slate-400 bg-purple-50 dark:bg-purple-900/20 px-3 py-1.5 rounded-lg"
                              >
                                {ref}
                              </li>
                            )
                    ...
                    {currentQuestion.answer_explanation.wrong_answer_explanations
                      ?.length > 0 && (
                      <div>
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2 flex items-center gap-2">
                        ...
                        <div className="space-y-2 pl-6">
                          {currentQuestion.answer_explanation.wrong_answer_explanations.map(
                            (we, idx) => (
                              <div
                                key={idx}
                                className="bg-rose-50 dark:bg-rose-900/10 border border-rose-200 dark:border-rose-800 rounded-lg p-3"
                              >
